...
### 캐시 정책
- 로컬 캐시를 적용하였습니다. 다중화 환경이나 캐시되는 데이터의 양이 많아질경우 글로벌캐시(Redis 등)을 사용하는것이 더 효율적입니다.
- 현재 상황에서는 상품정보가 빈번하게 변경되지 않을것으로 예상하여 만료시간을 1분으로 측정하여 캐시를 적용하였습니다.
- 상품정보 변경이 거의 발생하지 않는다면 만료시간을 높은 값으로 설정한 뒤 상품정보 변경시 캐시데이터를 갱신해주는 방식으로 변경할 수도 있습니다.
- 로컬 캐시 사용시 캐시데이터가 만료되는 시점에 트래픽이 많이 몰리는 상황이 발생할 경우 불필요한 캐싱이 발생하여 메모리 부족 현상이 발생할 수 있습니다.
